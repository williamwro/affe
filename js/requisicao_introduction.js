...
function mudarparcela() {
    ...
    var valor_total_mumero;
    valor_total_mumero = moedaParaNumero(valor_total);
    ...
    debugger;
    if (nparcela_escolhida === 1) {
        if (valor_total_mumero <= limitex) {
            $("#valor_parcela").html("");
            $("#val_parcela").val("");
            msg_parcela.css('display','none');
        }else {
            if (valor_total_mumero > 0 ) {
                Swal.fire({
                    title: "Atenção!",
        ...
        nparcela_escolhida = parseInt(nparcela_escolhida);
        valor_parcela = moedaParaNumero(valor_total_mumero / nparcela_escolhida);
